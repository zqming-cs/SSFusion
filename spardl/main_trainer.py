...
def robust_ssgd(dnn,
    ...
    for epoch in range(max_epochs):
        ...
        for i in range(iters_per_epoch):
            ...
            forward_times.append(trainer.forward_time)
            backward_times.append(trainer.backward_time)
            
            compression_times.append(optimizer._allreducer.compression_time)
            optimizer._allreducer.compression_time = 0
            communication_times.append(optimizer._allreducer.communication_time)
            optimizer._allreducer.communication_time = 0
            hook_time = optimizer._hook_time
            optimizer._hook_time = 0
            
            if i % display == 0 and i > 0 and rank == 0:
                time_per_iter = np.mean(times)
                forward_time_per_iter = np.mean(forward_times)
                backward_time_per_iter = np.mean(backward_times)
                compression_time_per_iter = np.mean(compression_times)
                communication_time_per_iter = np.mean(communication_times)
                io_time_per_iter = np.mean(trainer.iotime_array)
                logger.info(
                    'Time per iteration is %f. including communication: %f. forward: %f. backward: %f. compression: %f. io: %f. HOOK: %f. Speed: %f images/s, current density: %f',
                    time_per_iter,communication_time_per_iter, forward_time_per_iter,
                    backward_time_per_iter,
                    compression_time_per_iter,
                    io_time_per_iter,
                    hook_time,
