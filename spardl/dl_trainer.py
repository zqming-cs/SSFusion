...
_support_dnns = [
    'resnet50', 'resnet20', 'resnet56', 'resnet110', 'resnet152','vgg19', 'vgg16',
...
class DLTrainer:
    ...
    def train(self, num_of_iters=1, data=None, hidden=None):
        ...
        for i in range(num_of_iters):
            ...
            io_time = time.time() - ss
            self.iotime += io_time
            self.iotime_array.append(io_time)
            com_time = time.time()
            ...
            if self.dnn == 'lstm':
                ...
                self.forward_time = time.time() - com_time
                com_time = time.time()
                
                loss.backward()
                
                self.backward_time = time.time() - com_time
            elif self.dnn == 'lstmimdb':
